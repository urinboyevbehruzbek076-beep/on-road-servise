  ...
  if (step === 'landing') {          
    return (
      <div style={{ backgroundColor: '#0D0D0D', color: '#fff', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        ...
        <h1 style={{ color: '#FFB800', fontSize: '32px', margin: '0', letterSpacing: '3px', fontWeight: '900' }}>ON-ROAD</h1>
        <p style={{ color: '#aaa', textAlign: 'center', maxWidth: '280px', margin: '15px 0 40px' }}>{t('hero_title')}</p>
        <button onClick={() => setStep('role')} style={{ backgroundColor: '#FFB800', color: '#000', padding: '18px 60px', fontSize: '18px', fontWeight: '900', border: 'none', borderRadius: '50px', cursor: 'pointer', boxShadow: '0 10px 20px rgba(255, 184, 0, 0.3)' }}>{t('start')}</button>
      </div>
    );
  }
