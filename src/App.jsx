  ...
  if (step === 'landing') {          
    return (
      <div style={{ backgroundColor: '#0D0D0D', color: '#fff', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        ...
        <img src={logo} alt="Logo" style={{ width: '200px', height: '200px', objectFit: 'cover', borderRadius: '50%', border: '4px solid #FFB800', marginBottom: '20px', boxShadow: '0 0 30px rgba(255, 184, 0, 0.3)' }} />
        <h1 style={{ color: '#FFB800', fontSize: '32px', margin: '0', letterSpacing: '2px', fontWeight: '900' }}>ON ROAD SERVICE</h1>
  ...
  if (step === 'role') {
    return (
      <div style={{ backgroundColor: '#0D0D0D', color: '#fff', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
        <h2 style={{ marginBottom: '40px', color: '#FFB800', fontWeight: '900' }}>ON ROAD SERVICE</h2>
        <h3 style={{ marginBottom: '30px' }}>{t('choose_role')}</h3>
        <div style={{ display: 'flex', gap: '20px' }}>
          <button onClick={() => { setUserRole('user'); setStep('dashboard'); }} style={{ padding: '30px', border: '2px solid #FFB800', background: 'none', color: '#fff', borderRadius: '20px', cursor: 'pointer', width: '150px' }}>🚗<br/>{t('driver')}</button>
          <button onClick={() => { setUserRole('master'); setStep('dashboard'); }} style={{ padding: '30px', border: '2px solid #FFB800', background: 'none', color: '#fff', borderRadius: '20px', cursor: 'pointer', width: '150px' }}>🛠<br/>{t('master')}</button>
        </div>
      </div>
    );
  }
